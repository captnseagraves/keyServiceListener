...
app.post('/originEvents', async (req, res) => {
    try {
        ...
        const result = await db.one(
            'INSERT INTO key_service_originEvents(sender, user_op, transaction_hash, block_number) VALUES($1, $2, $3, $4) RETURNING id',
...
app.put('/originEvents/:id', async (req, res) => {
    try {
        const { sender, user_op, transaction_hash, block_number } = req.body;
        const result = await db.one(
            'UPDATE key_service_originEvents SET sender=$1, user_op=$2, transaction_hash=$3, block_number=$4 WHERE id=$5 RETURNING *',
...
app.post('/factories', async (req, res) => {
    try {
        ...
        const result = await db.one(
            'INSERT INTO key_service_factories(factoryAddress) VALUES($1, $2, $3, $4) RETURNING id',
            [factoryAddress]
        );
        res.json(result);
    } catch (error) {
